...
import com.atakmap.android.helloworld.compat.GeocodingTaskCompat;
...
public class SpeechNavigator extends SpeechActivity {
    ...
    void startActivity() {
        ...
        final GeocodingTaskCompat gt = new GeocodingTaskCompat(getPluginContext(),
                gb.getSouth(), gb.getWest(), gb.getNorth(),
                gb.getEast(), false);
        gt.setOnResultListener(new GeocodingTaskCompat.ResultListener() {
            @Override
            public void onResult() {
                if (gt.getPoint() != null) {
                    Log.d(TAG, "Inside GeocodingTaskCompat result listener");
    ...
    private void originFinder(String s) {
        GeoBounds gb = getView().getBounds();
        final GeocodingTaskCompat gt = new GeocodingTaskCompat(getView().getContext(),
                gb.getSouth(), gb.getWest(), gb.getNorth(),
                gb.getEast(), false);
        gt.setOnResultListener(new GeocodingTaskCompat.ResultListener() {
            @Override
